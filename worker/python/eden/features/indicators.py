...
def detect_liquidity_sweeps(df: pd.DataFrame, lookback: int = 5) -> pd.DataFrame:
    ...
    return pd.DataFrame({'sweep_high': sweep_high.astype(int), 'sweep_low': sweep_low.astype(int)}, index=df.index)


def identify_order_blocks(df: pd.DataFrame, window: int = 5, vol_mult: float = 1.5) -> pd.DataFrame:
    ...
    return pd.DataFrame({'ob_bull': bull_block.astype(int), 'ob_bear': bear_block.astype(int)}, index=df.index)
