...
def setup_logging(
    log_file: Optional[str] = None, debug: bool = False, verbose: bool = False
) -> logging.Logger:
...
class SystemMonitor:
    ...
    def __init__(
        ...
        monitor_subprocesses: bool = False,
        notify_on_stall: bool = False,
    ):
...
class EdenRunner:
    ...
    def __init__(self, args):
        ...
        self.monitor = SystemMonitor(
            args.monitor_cpu, args.monitor_subprocesses, args.notify_on_stall
        )
        ...
        try:
            ...
            if self.args.git_commit_message:
                ...
                def replace_date(match):
                    ...
                    py_format = (
                        format_str.replace("%Y", "%Y")
                        .replace("%m", "%m")
                        .replace("%d", "%d")
                    )
                    py_format = (
                        py_format.replace("%H", "%H")
                        .replace("%M", "%M")
                        .replace("%S", "%S")
                    )
    ...
    def _setup_strategies(self, df_features):
        ...
        if self.args.backtest_strategies:
            strategy_names = [
                s.strip().lower() for s in self.args.backtest_strategies.split(",")
            ]
        ...
        if not strategies:
            self.logger.warning(
                "No strategies configured, adding default momentum strategy"
            )
            strategies.append(MomentumStrategy())
    ...
    def _train_ml_model(self, df_features, model_path):
        ...
        if self.args.ml_optimization:
            ...
            if min_class_size >= 3:  # Minimum for 3-fold CV
                ...
                self.logger.warning(
                    f"Insufficient data for CV (min class size: {min_class_size}), using default parameters"
                )
        ...
        for strategy in strategies:
            try:
                ...
                if signals is not None and not signals.empty:
                    self.logger.info(
                        f"✅ Strategy {strategy.name} verified: {len(signals)} potential signals"
                    )
                    verified_strategies.append(strategy)
                else:
                    self.logger.warning(
                        f"⚠️ Strategy {strategy.name} produced no signals"
                    )
                    if not self.args.auto_retry:
                        verified_strategies.append(
                            strategy
                        )  # Include anyway if not auto-retry
            except Exception as e:
                self.logger.error(
                    f"❌ Strategy {strategy.name} failed verification: {e}"
                )
                if self.args.debug:
                    import traceback
            ...
            class DynamicMLStrategy(StrategyBase):
                ...
                def on_data(self, df):
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Dynamic strategy signal generation failed: {e}"
                        )

                    return pd.DataFrame(
                        signals, columns=["timestamp", "side", "confidence"]
                    )
    ...
    def _fetch_comprehensive_data(self, symbol, start_date, end_date, timeframes):
        ...
        self.logger.info(
            f"📊 Fetching comprehensive historical data from {start_date} to {end_date}"
        )
            ...
            for source in data_sources:
                try:
                    if source == "yfinance":
                        ...
                        self.logger.info(
                            f"Fetching {symbol} data via yfinance ({ticker})..."
                        )
                        ...
                        for interval, period in [
                            ...
                            try:
                                ...
                                if df is not None and not df.empty:
                                    ...
                                    if not out.empty:
                                        self.logger.info(
                                            f"✅ Fetched {len(out)} records from yfinance @ {interval}"
                                        )
                                        return out
                            except Exception as ie:
                                self.logger.debug(
                                    f"yfinance {interval} fetch failed: {ie}"
                                )
    ...
    def _generate_synthetic_data(self, symbol, start_date, end_date):
        ...
        self.logger.info(
            "📈 Generating synthetic market data for comprehensive testing..."
        )
    ...
    def _apply_feature_refinement(self, df_features):
        ...
        refinement_features = [
            f.strip().lower() for f in self.args.feature_refinement.split(",")
        ]
        ...
        try:
            ...
            if "liquidity_sweep" in refinement_features:
                ...
                if (
                    "high" in refined_features.columns
                    and "low" in refined_features.columns
                ):
                    refined_features["liquidity_sweep_strength"] = (
                        (refined_features["high"] - refined_features["low"])
                        / refined_features["close"]
                        * 100
                    )
                ...
                for col in ["1H_close", "4H_close", "1D_close"]:
                    if col in refined_features.columns:
                        refined_features[f"{col}_trend"] = (
                            refined_features[col]
                            > refined_features[col].rolling(20).mean()
                        ).astype(int)

            self.logger.info(
                f"✅ Applied {len(refinement_features)} feature refinements"
            )
    ...
    def _optimize_strategy_thresholds(self, strategies, df_features):
        ...
        try:
            ...
            for strategy in strategies:
                    ...
                    try:
                        ...
                        if signals is not None and not signals.empty:
                            ...
                            test_signals = signals[
                                signals.get("confidence", 1.0) >= threshold
                            ].copy()
                            ...
                            if len(test_signals) > 5:  # Minimum trades for valid test
                                ...
                                trades = engine.run(
                                    df_features, test_signals, symbol="VIX100"
                                )
                ...
                self.logger.info(
                    f"Strategy {strategy.name}: optimized threshold = {best_threshold:.2f} (Sharpe: {best_sharpe:.2f})"
                )
    ...
    def _apply_adaptive_weighting(self, strategies, df_features):
        ...
        if not (
            self.args.adaptive_strategy_weighting or self.args.ml_strategy_weighting
        ):
            return strategies
        ...
        try:
                ...
                try:
                    ...
                    if signals is not None and not signals.empty:
                        ...
                        if trades:
                            ...
                            if (
                                self.args.ml_strategy_weighting
                                and "ml" in strategy.name.lower()
                            ):
                                # Give ML strategies bonus weight if they perform well
                                ml_bonus = (
                                    0.3 if (sharpe > 0.5 and net_pnl > 0) else 0.0
                                )
                                score = (
                                    sharpe * 0.5
                                    + profit_factor * 0.25
                                ...
                                score = (
                                    sharpe * 0.4 + profit_factor * 0.3 + win_rate * 0.3
                                )

                            strategy_weights[strategy.name] = max(
                                0.1, score
                            )  # Minimum 10% weight
                        else:
            ...
            try:
                if getattr(
                    self.args, "weighting", None
                ) and self.args.weighting.lower().startswith("ppo"):
    ...
    def _apply_daily_adaptive_tuning(self, df_features):
        ...
        try:
            ...
            recent_dates = (
                unique_dates[-30:] if len(unique_dates) >= 30 else unique_dates
            )
                ...
                if len(daily_data) > 0:
                    ...
                    try:
                        ...
                        if len(X) > 10:  # Minimum samples
                            ...
                            daily_model_path = (
                                models_dir / f"daily_adapted_{date}.joblib"
                            )
        ...
        try:
            ...
            class EvolvedStrategy(StrategyBase):
                def __init__(self, base_strategy, mutation_params):
                    self.name = (
                        f"evolved_{base_strategy.name}_{random.randint(1000,9999)}"
                    )
                ...
                def on_data(self, df):
                    ...
                    confidence_multiplier = self.mutation_params.get(
                        "confidence_mult", 1.0
                    )
                    if "confidence" in evolved_signals.columns:
                        evolved_signals["confidence"] *= confidence_multiplier
                        evolved_signals["confidence"] = evolved_signals[
                            "confidence"
                        ].clip(0.1, 0.95)
            ...
            self.logger.info(
                f"✅ Generated {len(evolved_strategies) - len(strategies)} evolved strategies"
            )
    ...
    def _apply_htf_ict_bias(self, signals, df_features):
        ...
        try:
            ...
            df_htf["htf_bias_score"] = (
                df_htf["htf_trend"] + df_htf["htf_momentum"]
            ) / 2
            ...
            if not signals.empty:
                ...
                self.logger.info(
                    f"✅ HTF bias applied: {len(signals)} -> {len(biased_signals)} signals"
                )
    ...
    def _postprocess_comprehensive_metrics(self, metrics, trades, output_dir):
        ...
        try:
            ...
            if trades:
                ...
                if "entry_time" in trades_df.columns:
                    daily_returns = trades_df.groupby(trades_df["entry_time"].dt.date)[
                        "pnl"
                    ].sum()
                else:
                ...
                if len(daily_returns) > 1:
                    ...
                    comprehensive_metrics["risk_adjusted_return"] = (
                        comprehensive_metrics.get("net_pnl", 0)
                        / comprehensive_metrics["volatility"]
                        if comprehensive_metrics["volatility"] > 0
                        else 0
                    )
                ...
                if "strategy" in trades_df.columns:
                    ...
                    comprehensive_metrics["strategy_contributions"] = (
                        strategy_pnl.to_dict()
                    )
    ...
    def _generate_enhanced_equity_curve(self, analyzer, output_dir):
        ...
        try:
            ...
            equity_curve = (
                ...
                if callable(getattr(analyzer, "equity_curve", None))
                else analyzer.equity_curve
            )
            ax1.plot(
                equity_curve.index,
                equity_curve["equity"],
            ...
            drawdown = (
                ...
                if callable(getattr(analyzer, "drawdown_curve", None))
                else analyzer.drawdown_curve
            )
            ax2.fill_between(
                drawdown.index,
                drawdown["drawdown"],
    ...
    def _filter_signals_with_ml(self, df_features, signals):
        ...
        try:
            ...
            if getattr(self.args, "ensemble_filter", None):
                selected = {
                    s.strip().lower() for s in self.args.ensemble_filter.split(",")
                }
            ...
            if use_xgb:
                try:
                    ...
                    target = (
                        (df_features["close"].pct_change().shift(-1) > 0)
                        .astype(int)
                        .reindex(X.index)
                        .fillna(0)
                    )
            ...
            if not signals.empty:
                ...
                sig["ensemble_prob"] = sig["ensemble_prob"].fillna(
    ...
    def run_phase3_weekly_profit_loop(self):
        ...
        if self.args.fetch_full_historical_data:
            exec_tfs = (
                self.args.execution_tf.split(",")
                if self.args.execution_tf
                else ["M1", "M5"]
            )
            top_tfs = (
                self.args.topdown_tf.split(",")
                if self.args.topdown_tf
                else ["M15", "1H", "4H"]
            )
            df = self._fetch_comprehensive_data(
        ...
        if (
            getattr(self.args, "anomaly_filter", None)
            and self.args.anomaly_filter.lower() == "isolationforest"
        ):
        ...
        df_features["week"] = (
            pd.to_datetime(df_features.index).to_period("W").astype(str)
        )
        ...
        for wk in weeks:
            ...
            weekly_profit_ratio = (
                (weekly_pnl / starting_cash) if starting_cash > 0 else 0.0
            )
            ...
            weekly_summary.append(
                {
                    "week": wk,
                    "pnl": weekly_pnl,
                    "profit_ratio": weekly_profit_ratio,
                    "dd": weekly_dd,
            ...
            if weekly_profit_ratio < target:
                ...
                self.htf_bias_threshold = max(
                    0.2, getattr(self, "htf_bias_threshold", 0.5) - 0.05
                )

            # Stop if drawdown exceeded
            if self.args.drawdown_control == "enabled" and weekly_dd > max_dd:
                self.logger.warning(
                    f"Weekly drawdown {weekly_dd:.2%} exceeded limit {max_dd:.2%}; stopping loop"
                )
                break

            # Git checkpoint per week
            self.create_git_checkpoint(
                "weekly_cycle", f"Week {wk} complete - profit {weekly_profit_ratio:.2%}"
            )
    ...
    def run_continuous_loop(self):
        ...
        while True:
            ...
            try:
                ...
                df = None
                if (
                    self.args.offline_only or self.args.use_local_data_if_available
                ) and self.args.store_local_data:
                    try:
                        df = dl.load_csv(Path(self.args.store_local_data))
                        self.logger.info(
                            f"Loaded local data: {self.args.store_local_data}"
                        )
                    except Exception as e:
                ...
                (cycle_dir / "report.json").write_text(
                    json.dumps({**report, "metrics": metrics}, indent=2, default=str)
                )
                ...
                self.logger.info(
                    f"=== Cycle {cycle} — Complete — trades: {len(trades or [])}, pnl: {metrics.get('net_pnl',0)} ==="
                )
            except Exception as e:
    ...
    def run_phase3_ml_ict(self):
        ...
        if self.args.fetch_full_historical_data:
            timeframes = (
                self.args.multi_timeframe.split(",")
                if self.args.multi_timeframe
                else ["M1", "M5", "15M", "1H", "4H"]
            )
            df = self._fetch_comprehensive_data(
                "Volatility 100 Index",
                self.args.start_date,
        ...
        if self.args.store_local_data and not (
            Path(self.args.store_local_data).exists()
        ):
        ...
        if self.args.multi_timeframe:
            htf_timeframes = [
                tf.strip()
                for tf in self.args.multi_timeframe.split(",")
                if tf.strip() != "M1"
            ]
        else:
        ...
        if self.args.ml_strategy_generation:
            ...
            self.logger.info(
                f"Added {len(ml_generated_strategies)} ML-generated strategies"
            )
        ...
        self.create_git_checkpoint(
            "strategies_ready", f"Strategies configured: {[s.name for s in strategies]}"
        )
        ...
        if self.args.grid_optimization:
            ...
            self.create_git_checkpoint(
                "optimization_complete", "Grid optimization completed"
            )
            ...
            try:
                ...
                if signals is not None and not signals.empty:
                    ...
                    if hasattr(signals, "confidence"):
                        signals = signals[
                            signals["confidence"] >= self.args.ml_threshold
                        ]
        ...
        if self.args.optimization_results_json and hasattr(
        ...
        self.create_git_checkpoint(
            "phase3_complete",
            f"Phase 3 completed - PnL: ${metrics.get('net_pnl', 0):.2f}, Trades: {metrics.get('trades', 0)}",
        )
    ...
    def run(self):
        ...
        try:
            ...
            self.logger.info(
                f"Execution completed in {time.time() - self.start_time:.1f}s"
            )
            return 0
...
def main():
    ...
    # Phase selection
    parser.add_argument(
        "--phase3", action="store_true", help="Run Phase 3 ML ICT strategy"
    )
    ...
    parser.add_argument(
        "--incremental-days", action="store_true", help="Process data incrementally"
    )

    # ML and strategy parameters
    parser.add_argument(
        "--ml-enabled", action="store_true", help="Enable ML components"
    )
    parser.add_argument(
        "--ml-ict-filter", action="store_true", help="Apply ML ICT filtering"
    )
    parser.add_argument(
        "--ml-extensive-optimization",
        action="store_true",
        help="Enable extensive ML optimization",
    )
    parser.add_argument(
        "--ml-strategy-generation",
        action="store_true",
        help="Enable ML-based strategy generation",
    )
    parser.add_argument(
        "--ml-daily-adaptive-tuning",
        action="store_true",
        help="Enable daily adaptive ML tuning",
    )
    parser.add_argument(
        "--ml-threshold", type=float, default=0.6, help="ML confidence threshold"
    )
    # ICT tuning
    parser.add_argument(
        "--ict-stop-atr", type=float, default=1.0, help="ICT stop ATR multiplier"
    )
    parser.add_argument(
        "--ict-tp-atr", type=float, default=3.0, help="ICT take-profit ATR multiplier"
    )
    parser.add_argument(
        "--ict-killzones-enabled",
        action="store_true",
        help="Enable ICT kill zone gating",
    )
    parser.add_argument(
        "--ict-killzones",
        type=str,
        default="london,ny",
        help="Comma list of kill zones: london,ny",
    )
    parser.add_argument(
        "--ict-kz-london",
        type=str,
        default="07-10",
        help="London killzone window as HH-HH (UTC)",
    )
    parser.add_argument(
        "--ict-kz-ny",
        type=str,
        default="12-16",
        help="New York killzone window as HH-HH (UTC)",
    )
    parser.add_argument(
        "--backtest-strategies",
        type=str,
        help="Comma-separated list of strategies to backtest",
    )
    parser.add_argument(
        "--dynamic-risk-per-trade",
        type=float,
        default=0.02,
        help="Dynamic risk percentage per trade",
    )
    parser.add_argument(
        "--min-trade-value", type=float, default=0.5, help="Minimum trade value"
    )
    parser.add_argument(
        "--verify-strategy-functionality",
        action="store_true",
        help="Verify all strategies work before execution",
    )
    ...
    parser.add_argument(
        "--feature-refinement",
        type=str,
        help="Comma-separated list of features to refine (e.g., liquidity_sweep,fvg,htf_bias)",
    )
    parser.add_argument(
        "--adaptive-strategy-weighting",
        action="store_true",
        help="Enable adaptive strategy weighting based on performance",
    )
    parser.add_argument(
        "--risk-reward-tuning",
        action="store_true",
        help="Enable risk-reward ratio tuning",
    )
    parser.add_argument(
        "--strategy-threshold-optimizer",
        action="store_true",
        help="Optimize strategy confidence thresholds",
    )
    parser.add_argument(
        "--daily-performance-feedback",
        action="store_true",
        help="Enable daily performance feedback loop",
    )
    parser.add_argument(
        "--auto-ml-strategy-evolution",
        action="store_true",
        help="Enable automatic ML strategy evolution",
    )
    ...
    parser.add_argument(
        "--ml-fix-features",
        action="store_true",
        help="Automatically align ML features with strategy pipeline",
    )
    parser.add_argument(
        "--ml-optimization",
        action="store_true",
        help="Run advanced hyperparameter tuning for ML models",
    )
    parser.add_argument(
        "--ml-strategy-weighting",
        action="store_true",
        help="Dynamically adjust strategy contribution based on performance",
    )
    parser.add_argument(
        "--htf-ict-bias",
        action="store_true",
        help="Apply HTF bias to all strategy entries",
    )
    parser.add_argument(
        "--postprocess-metrics",
        action="store_true",
        help="Compute comprehensive performance metrics",
    )
    parser.add_argument(
        "--metrics-full", action="store_true", help="Alias for postprocess metrics"
    )
    parser.add_argument(
        "--equity-curve-plot",
        action="store_true",
        help="Generate equity curve visualization",
    )
    parser.add_argument(
        "--equity-curve", action="store_true", help="Alias for equity curve plot"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--frequency-priority",
        type=str,
        choices=["high", "ultra-high", "balanced", "conservative"],
        help="Prioritize trade frequency vs purity",
    )
    parser.add_argument(
        "--trade-thresholds",
        type=str,
        choices=["lower", "default", "higher", "adaptive-relaxed"],
        help="Adjust trade thresholds for ML and filters",
    )
    parser.add_argument(
        "--drawdown-control",
        type=str,
        choices=["enabled", "disabled", "moderate"],
        default="enabled",
        help="Enable weekly drawdown control",
    )
    parser.add_argument(
        "--weekly-target",
        type=float,
        help="Weekly profit target as fraction (e.g., 0.5 for 50%)",
    )
    parser.add_argument(
        "--max-drawdown-per-week",
        type=float,
        help="Max weekly drawdown as fraction (e.g., 0.2 for 20%)",
    )
    parser.add_argument(
        "--trades-csv", action="store_true", help="Export trades CSV explicitly"
    )
    ...
    parser.add_argument(
        "--continuous-loop",
        action="store_true",
        help="Run an infinite monitoring/learning loop",
    )
    parser.add_argument(
        "--cycle-interval",
        type=int,
    ...
    parser.add_argument(
        "--offline-only",
        action="store_true",
        help="Disallow external API calls; use only local/cached data",
    )
    parser.add_argument(
        "--loop-until-target",
        action="store_true",
        help="Alias to run continuous loop until target met",
    )
    ...
    parser.add_argument(
        "--ensemble-filter",
        type=str,
        help="Comma-separated list of ensemble filters to apply (e.g., XGBoost,LSTM-proxy)",
    )
    parser.add_argument(
        "--anomaly-filter",
        type=str,
        help="Anomaly filter to use (e.g., IsolationForest)",
    )
    parser.add_argument(
        "--weighting",
        type=str,
        help="Strategy weighting policy (e.g., PPO-style, heuristic, none)",
    )

    # Data management
    parser.add_argument(
        "--mt5-online", action="store_true", help="Use MT5 for live data"
    )
    parser.add_argument("--store-local-data", type=str, help="Path to store local data")
    parser.add_argument(
        "--offline-data-cache", type=str, help="Alias for store-local-data"
    )
    parser.add_argument(
        "--offline-data", type=str, help="Alias for store-local-data (file path)"
    )
    parser.add_argument(
        "--use-local-data-if-available",
        action="store_true",
        help="Use local data if available",
    )
    parser.add_argument(
        "--fetch-full-historical-data",
        action="store_true",
        help="Fetch comprehensive historical data",
    )
    parser.add_argument(
        "--execution-tf",
        type=str,
        help="Comma-separated execution timeframes (e.g., M1,M5)",
    )
    parser.add_argument("--execution-tfs", type=str, help="Alias for execution-tf")
    parser.add_argument(
        "--topdown-tf",
        type=str,
        help="Comma-separated higher timeframes (e.g., M15,1H,4H)",
    )
    parser.add_argument("--topdown-tfs", type=str, help="Alias for topdown-tf")
    parser.add_argument(
        "--multi-timeframe",
        type=str,
        help="Comma-separated list of timeframes to analyze (deprecated)",
    )

    # Optimization controls
    parser.add_argument(
        "--grid-optimization", action="store_true", help="Enable grid optimization"
    )
    parser.add_argument(
        "--grid-optimization-budget",
        type=int,
        default=20,
        help="Budget for grid optimization runs",
    )
    parser.add_argument(
        "--grid-budget", type=int, help="Alias for grid-optimization-budget"
    )
    parser.add_argument(
        "--walk-forward-windows",
        type=int,
        help="Limit number of weekly windows in walk-forward loop",
    )
    ...
    parser.add_argument(
        "--auto-retry", action="store_true", help="Auto-retry on failures"
    )
    parser.add_argument(
        "--retrain-if-stuck", action="store_true", help="Retrain models if needed"
    )
    ...
    parser.add_argument(
        "--monitor-subprocesses", action="store_true", help="Monitor subprocesses"
    )
    parser.add_argument(
        "--notify-on-stall", action="store_true", help="Notify on system stalls"
    )
    ...
    parser.add_argument(
        "--git-checkpoints",
        action="store_true",
        help="Create git checkpoints during execution",
    )
    parser.add_argument(
        "--git-commit-message", type=str, help="Custom git commit message template"
    )
