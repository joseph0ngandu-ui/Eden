...
class StatusCard(ttk.Frame):
    ...
    def __init__(
        self, parent, title: str, value: str = "", color: str = "neutral", **kwargs
    ):
        super().__init__(parent, style="Card.TFrame", **kwargs)

        self.title_label = ttk.Label(
            self, text=title, style="Card.TLabel", font=("Segoe UI", 10, "bold")
        )
        self.title_label.pack(anchor="w", padx=12, pady=(8, 2))

        self.value_label = ttk.Label(
            self, text=value, style="Card.TLabel", font=("Segoe UI", 16, "bold")
        )
...
class ProgressCard(ttk.Frame):
    ...
    def __init__(self, parent, title: str, **kwargs):
        ...
        self.status_label = ttk.Label(
            self, text="Ready", style="Card.TLabel", font=("Segoe UI", 9)
        )
...
class TradingChart(ttk.Frame):
    ...
    def __init__(self, parent, **kwargs):
        ...
        self.figure = Figure(
            figsize=(12, 6), dpi=100, facecolor="#21262D", edgecolor="none"
        )
    ...
    def plot_ohlcv(self, data: pd.DataFrame, symbol: str = ""):
        ...
        self.axis.set_title(
            f"{symbol} Price Chart" if symbol else "Price Chart",
            color="#F0F6FC",
            fontsize=14,
            pad=20,
        )
    ...
    def plot_performance(self, returns: pd.Series, benchmark: pd.Series = None):
        ...
        self.axis.set_title(
            "Performance Comparison", color="#F0F6FC", fontsize=14, pad=20
        )
        self.axis.set_xlabel("Date", color="#F0F6FC")
        self.axis.set_ylabel("Cumulative Return", color="#F0F6FC")
        self.axis.grid(True, alpha=0.3, color="#30363D")
        self.axis.tick_params(colors="#F0F6FC")
        self.axis.legend(facecolor="#161B22", edgecolor="#30363D", labelcolor="#F0F6FC")
...
class LogViewer(ttk.Frame):
    ...
    def __init__(self, parent, **kwargs):
        ...
        ttk.Button(toolbar, text="Clear", command=self.clear_logs).pack(side="right")
        ...
        self.text_widget = tk.Text(
            text_frame, wrap="word", height=10, font=("Consolas", 10)
        )
...
class ConfigForm(ttk.Frame):
    ...
    def create_form(self):
        ...
        for key, config in self.config_schema.items():
            ...
            # Label
            ttk.Label(self, text=f"{label_text}:", style="Panel.TLabel").grid(
            ...
            if field_type == "boolean":
                ...
                widget = ttk.Combobox(
                    self, textvariable=var, values=options, state="readonly"
                )
