...
class MTFDataFetcher:
    ...
    def _init_mt5(self):
        ...
        try:
            ...
            logger.warning(f"VIX100 symbol not found in MT5. Tried: {self.symbol_alternatives}")
            return False
    ...
    def fetch_timeframe(
        ...
        """
        # Check cache first
        cache_file = self.cache_dir / f"{self.symbol.replace(' ', '_')}_{timeframe}_{start_date.date()}_{end_date.date()}.csv"
        ...
        try:
            ...
            logger.info(f"Fetching {self.symbol} {timeframe} from {start_date} to {end_date}")
            ...
            if rates is None or len(rates) == 0:
                logger.error(f"No data returned for {timeframe}: {self.mt5.last_error()}")
                return None
            ...
            # Save raw data
            try:
                raw_file = self.raw_dir / f"{self.symbol.replace(' ', '_')}_{timeframe}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
                df.to_csv(raw_file)
            except Exception as e:
    ...
    def fetch_all_timeframes_range(
        ...
        logger.info(f"Fetching timeframes {timeframes} between {start_date} and {end_date}")
