...
def run_optimal_backtest(
    ...
    strategies = [
        config["primary_strategy"]["name"],
        config["secondary_strategy"]["name"],
    ]
    ...
    print(
        f"⏰ Timeframes: {'/'.join(exec_tfs)} execution, {'/'.join(htf_tfs)} HTF bias"
    )
    print(
        f"🎲 Risk: {config['risk_management']['per_order_risk_fraction']*100}% per trade, ${config['risk_management']['min_trade_value']} minimum"
    )
    print()
    ...
    if primary_key in consolidated and secondary_key in consolidated:
        ...
        combined_growth = (
            combined_pnl / config["account_settings"]["starting_cash"]
        ) * 100
    ...
    print(
        f"  📅 Weekly Review Required: {config['monitoring']['weekly_review_required']}"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run Eden Optimal Micro Account Configuration"
    )
    parser.add_argument(
        "--days-back", type=int, default=7, help="Days of historical data"
    )
    parser.add_argument("--start-date", type=str, help="Start date (YYYY-MM-DD)")
    parser.add_argument("--end-date", type=str, help="End date (YYYY-MM-DD)")
    parser.add_argument(
        "--output-dir", type=str, default="results_optimal", help="Output directory"
    )
