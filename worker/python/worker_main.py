...
from eden.config import load_config, EdenConfig
...
class EdenWorker:
    ...
    def start(self):
        ...
        self.running = True
        self.logger.info(f"Eden Worker started on ports REQ:{self.req_port}, PUB:{self.pub_port}")
    ...
    def handle_command(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if command == "ping":
                return {"status": "success", "response": "pong", "request_id": request_id}
                ...
                return {"status": "success", "response": "Shutting down", "request_id": request_id}
    ...
    def get_status(self, request_id: str) -> Dict[str, Any]:
        ...
        status = {
            ...
            "data_cache_size": len(list(Path("shared/data/cache").glob("*.csv"))) if Path("shared/data/cache").exists() else 0,
        }
    ...
    def _run_backtest_thread(self, params: Dict[str, Any], request_id: str):
        ...
        try:
            ...
            self.publish_progress(0.6, f"PF: {pf:.2f}, Max losing streak: {mcl}", request_id)

            # If 'optimize_as_you_go' flag is set, run a quick sweep and stream updates
            if params.get("optimize_as_you_go", True):
                try:
                    self.publish_progress(0.65, "Optimizing parameters (quick sweep)...", request_id)
                    ...
                    self.publish_progress(0.9, f"Best trial score: {quick.get('best_value', 0):.3f}", request_id)
                except Exception as e:
                    self.publish_status("opt_error", f"Optimization error: {e}")

            self.publish_progress(1.0, "Monte Carlo backtest complete", request_id)
            self.publish_status("backtest_complete", f"Backtest {request_id} completed successfully")
            
        except Exception as e:
            self.logger.error(f"Backtest error: {e}")
            self.publish_status("backtest_error", f"Backtest {request_id} failed: {str(e)}")
    ...
    def load_market_data(self, params: Dict[str, Any], request_id: str) -> Dict[str, Any]:
        ...
        try:
            df = self.data_loader.get_ohlcv(symbol, timeframe, start_date, end_date, prefer_mt5=prefer_mt5)
    ...
    def get_available_strategies(self, request_id: str) -> Dict[str, Any]:
        ...
        strategies = [
            {"id": "ict", "name": "ICT Strategy", "description": "Inner Circle Trader methodology"},
            {"id": "mean_reversion", "name": "Mean Reversion", "description": "Statistical mean reversion strategy"},
            {"id": "momentum", "name": "Momentum", "description": "Trend following momentum strategy"},
            {"id": "ensemble", "name": "Ensemble", "description": "Combined strategy approach"},
            {"id": "ml_generated", "name": "ML Generated", "description": "Machine learning generated strategy"}
        ]
    ...
    def optimize_strategy(self, params: Dict[str, Any], request_id: str) -> Dict[str, Any]:
    ...
    def run_parameter_sweep(self, params: Dict[str, Any], request_id: str) -> Dict[str, Any]:
        """Explicit endpoint for parameter sweeps."""
        return self.optimize_strategy(params, request_id)

    def train_regime_classifier(self, params: Dict[str, Any], request_id: str) -> Dict[str, Any]:
        """Placeholder for regime classifier training in Phase 2."""
        return {"status": "success", "response": {"message": "Regime training scheduled"}, "request_id": request_id}

    def train_trade_scorer(self, params: Dict[str, Any], request_id: str) -> Dict[str, Any]:
        """Placeholder for trade scoring model training in Phase 2."""
        return {"status": "success", "response": {"message": "Trade scorer training scheduled"}, "request_id": request_id}
