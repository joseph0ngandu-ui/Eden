...
class StatusCard(QFrame):
    ...
    def __init__(
        self, title: str, value: str = "0", color: str = "neutral", parent=None
    ):
    ...
    def __init__(self, parent=None):
        ...
        if PYQTGRAPH_AVAILABLE:
            ...
            self.plot_widget.setLabel(
                "left", "Price", color=EdenTheme.TEXT_SECONDARY, size="11pt"
            )
            self.plot_widget.setLabel(
                "bottom", "Time", color=EdenTheme.TEXT_SECONDARY, size="11pt"
            )

            # Apply Apple-style plot styling
            self.plot_widget.getPlotItem().getAxis("left").setPen(
                EdenTheme.BORDER_PRIMARY
            )
            self.plot_widget.getPlotItem().getAxis("bottom").setPen(
                EdenTheme.BORDER_PRIMARY
            )
            self.plot_widget.getPlotItem().getAxis("left").setTextPen(
                EdenTheme.TEXT_SECONDARY
            )
            self.plot_widget.getPlotItem().getAxis("bottom").setTextPen(
                EdenTheme.TEXT_SECONDARY
            )
            ...
            placeholder.setStyleSheet(
                f"color: {EdenTheme.TEXT_MUTED}; font-size: 16px;"
            )
    ...
    def plot_ohlcv_data(self, data: pd.DataFrame, symbol: str = ""):
        ...
        if symbol:
            self.plot_widget.setTitle(
                f"{symbol} Price Chart", color=EdenTheme.TEXT_PRIMARY, size="14pt"
            )
...
class EdenDashboard(QMainWindow):
    ...
    def create_trading_tab(self):
        ...
        header.setStyleSheet(
            f"color: {EdenTheme.TEXT_PRIMARY}; font-size: 20px; font-weight: 600;"
        )
        ...
        self.mt5_status_label.setStyleSheet(
            f"color: {EdenTheme.TEXT_MUTED}; font-size: 12px;"
        )
        ...
        self.account_info_label.setStyleSheet(
            f"color: {EdenTheme.TEXT_SECONDARY}; padding: 10px;"
        )
    ...
    def create_backtest_tab(self):
        ...
        self.mc_run_btn.setToolTip(
            "Bootstrap simulations of equity drawdowns from backtest trades. Not run automatically."
        )
    ...
    def create_logs_tab(self):
        ...
        header.setStyleSheet(
            f"font-size: 20px; font-weight: 600; color: {EdenTheme.TEXT_PRIMARY};"
        )
        header_layout.addWidget(header)
    ...
    def load_initial_data(self):
        ...
        if self.log_console:
            self.log_console.add_log(
                "Eden Bot Professional Dashboard initialized", "SUCCESS"
            )
    ...
    def run_backtest(self):
        ...
        if self.log_console:
            self.log_console.add_log(
                f"Starting backtest for {symbol} {timeframe} with {strategy} strategy",
                "INFO",
            )
            ...
            try:
                ...
                def _update_success():
                    ...
                    if self.log_console:
                        ...
                        self.log_console.add_log(
                            "Artifacts written to examples/results (metrics.json, equity_curve.png, trades.csv)",
                            "INFO",
                        )
    ...
    def complete_backtest(self):
        ...
        if self.log_console:
            self.log_console.add_log(
                "Backtest completed with sample results", "SUCCESS"
            )
            self.log_console.add_log(
                "Total Return: +15.5%, Sharpe: 1.2, Max DD: -8.3%", "INFO"
            )
    ...
    def run_monte_carlo(self):
        ...
        if self.log_console:
            self.log_console.add_log(
                "Monte Carlo wired: will use backtest trades when available. Not running automatically.",
                "INFO",
            )
    ...
    def discover_strategies(self):
        ...
        if self.log_console:
            ...
            self.log_console.add_log(
                "This feature requires the full Eden backend", "WARNING"
            )
    ...
    def toggle_trading(self, enabled: bool):
        ...
        if enabled:
            if self.mt5_manager.enable_trading():
                if self.log_console:
                    self.log_console.add_log(
                        "✅ Trading enabled - Eden can now place trades", "SUCCESS"
                    )
            else:
    ...
    def on_mt5_connection_change(self, status, account_info):
        ...
        if status.value == "Connected" and account_info:
            ...
            self.mt5_status_label.setStyleSheet(
                f"color: {EdenTheme.SUCCESS}; font-size: 12px;"
            )
...
Leverage: 1:{account_info.leverage}"""
            ...
            # Update status label color
            self.mt5_status_label.setStyleSheet(
                f"color: {EdenTheme.DANGER}; font-size: 12px;"
            )
