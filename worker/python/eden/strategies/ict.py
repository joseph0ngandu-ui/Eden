...
class ICTStrategy(StrategyBase):
    ...
    def order_block_retest(self, df: pd.DataFrame, idx: int) -> Optional[dict]:
        ...
        micro_imbalance_bull = self._col_scalar(df, 'micro_imbalance_bull', idx, default=0)
        micro_imbalance_bear = self._col_scalar(df, 'micro_imbalance_bear', idx, default=0)
    ...
    def calculate_stop_tp(self, df: pd.DataFrame, idx: int, side: str) -> dict:
        ...
        if atr.iloc[idx] == 0 or pd.isna(atr.iloc[idx]):
            ...
            atr_val = (df['high'].iloc[max(0,idx-14):idx+1].max() - 
                      df['low'].iloc[max(0,idx-14):idx+1].min()) / 14
        else:
    ...
    def on_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if signals:
            ...
            required_cols = ['timestamp', 'side', 'confidence', 'tag', 'htf_bias', 
                           'stop_price', 'tp_price', 'atr']
            for col in required_cols:
                if col not in result.columns:
                    result[col] = 0 if col in ['htf_bias'] else np.nan
            return result
        else:
            return pd.DataFrame(columns=['timestamp', 'side', 'confidence', 'tag', 
                                        'htf_bias', 'stop_price', 'tp_price', 'atr'])
