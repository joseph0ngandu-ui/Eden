...
class EdenInstaller:
    ...
    def create_ui(self):
        ...
        style.configure('Subtitle.TLabel', background=bg_color, foreground=secondary_fg, 
                       font=('-apple-system', 'SF Pro Text', 16))
        style.configure('TButton', background=primary_color, foreground='#FFFFFF', 
                       font=('-apple-system', 'SF Pro Text', 13, 'normal'),
                       relief='flat', borderwidth=0)
        style.map('TButton', background=[('active', primary_hover), ('pressed', primary_pressed)])
        style.configure('TProgressbar', background=primary_color, troughcolor=trough_color,
                       borderwidth=0, lightcolor=primary_color, darkcolor=primary_color)
        ...
        subtitle_label = ttk.Label(header_frame, text="The Origin of Order", style='Subtitle.TLabel')
        subtitle_label.pack(pady=(0, 16))
        
        description_label = ttk.Label(header_frame, text=self.app_description, style='TLabel')
        ...
        ttk.Label(info_frame, text=f"Version: {self.app_version}", style='TLabel').pack(anchor='w')
        ttk.Label(info_frame, text=f"Publisher: {self.publisher}", style='TLabel').pack(anchor='w')
        ttk.Label(info_frame, text=f"Install Location: {self.install_dir}", style='TLabel').pack(anchor='w')
        ...
        ttk.Label(progress_frame, text="Installation Progress:", style='TLabel').pack(anchor='w')
        
        self.progress_bar = ttk.Progressbar(progress_frame, variable=self.progress_var, maximum=100)
        self.progress_bar.pack(fill='x', pady=(5, 5))
        
        self.status_label = ttk.Label(progress_frame, textvariable=self.status_var, style='TLabel')
        ...
        self.log_text = tk.Text(log_frame, height=8, bg=log_bg, fg=log_fg, 
                               font=('-apple-system', 'SF Mono', 12), yscrollcommand=log_scroll.set,
    ...
    def install_python_packages(self):
        ...
        try:
            ...
            for i, package in enumerate(self.required_packages):
                ...
                self.update_progress(30 + (i / total_packages) * 40, 
                ...
                try:
                    ...
                    if result.returncode != 0:
                        self.log(f"Warning: Failed to install {package}: {result.stderr}", "WARNING")
                    else:
