    ...
    def run(self, df: pd.DataFrame, signals: pd.DataFrame, symbol: str, risk_manager=None) -> List[Trade]:
        ...
        for ts, price in price_series.items():
            ...
            while sig_idx < len(sig_rows) and sig_rows[sig_idx]["timestamp"] <= ts:
                ...
                atr_val = float(row.get('atr', df_row['atr_14'] if df_row is not None and 'atr_14' in df_row else 0.0))
                ...
                risk_per_trade = max(self.equity * self.per_order_risk_fraction, self.min_trade_value)
                ...
                conf = float(np.real(conf)) if isinstance(conf, complex) else float(conf)
                base_qty = float(np.real(base_qty)) if isinstance(base_qty, complex) else float(base_qty)
