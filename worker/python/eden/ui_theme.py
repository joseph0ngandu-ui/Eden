...
def apply_theme(root: tk.Tk, theme: Dict[str, Any], **kwargs):
    ...
    try:
        ...
        style.configure(
            ...
            foreground=fg,
            font=(
                theme.get("font_family", "Segoe UI"),
                theme.get("body_font_size", 11),
            ),
        )

        # Configure panels and cards
        style.configure(
            "Panel.TFrame", background=panel_bg, relief="flat", borderwidth=1
        )
        ...
        style.configure(
            ...
            font=(
                theme.get("font_family", "Segoe UI"),
                theme.get("subheader_font_size", 16),
                "bold",
            ),
        )
        ...
        style.configure(
            "Secondary.TButton", background=accent_secondary, foreground="white"
        )
        style.map("Secondary.TButton", background=[("active", hover_bg)])

        # Configure danger buttons
        style.configure("Danger.TButton", background=accent_danger, foreground="white")
        ...
        style.map("TRadiobutton", background=[("active", hover_bg)])
