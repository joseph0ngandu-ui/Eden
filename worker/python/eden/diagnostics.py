        ...
        try:
            system_info = {
                ...
                'disk': {
                    ...
                    'percent_used': psutil.disk_usage('/').percent if os.name != 'nt' else psutil.disk_usage('C:\\').percent,
                },
                'cpu': {
...
class CrashReporter:
    ...
    def generate_crash_report(self, exc_type, exc_value, exc_traceback):
        ...
        # Save crash report
        crash_file = f"crash_report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
