...
def compute_metrics_for_run(run_dir: Path) -> Dict:
    ...
    start_equity = (
        float(trades.get("starting_cash").iloc[0])
        if "starting_cash" in trades.columns and not trades["starting_cash"].empty
        else 100000.0
    )
    ...
    metrics = {
        ...
        "avg_trade_duration_min": float(
            trades["duration"].mean() if not trades.empty else 0.0
        ),
        ...
        "risk_adjusted_return_per_dollar": float(
            net_pnl / start_equity if start_equity > 0 else 0.0
        ),
...
def main(results_root: Path = Path("results")) -> Dict:
    ...
    (results_root / "consolidated_report.json").write_text(
