...
class DataLoader:
    ...
    def fetch_yfinance(
        ...
        *,
        ...
        if yf is None or not allow_network:
            self.log.warning(
                "yfinance unavailable or network disabled; returning empty DataFrame for %s",
                symbol,
            )
            return None
        try:
            ...
            out_reset = out.reset_index().rename(
    ...
    def fetch_stooq_daily(
        self, symbol: str, *, allow_network: bool = True
    ) -> Optional[pd.DataFrame]:
        if not allow_network:
            return None
        ...
        try:
            ...
            if (
                r.status_code != 200
                or not r.text
                or r.text.lower().startswith("brak danych")
            ):
                return None
    ...
    def _cache_store(
        self, symbol: str, timeframe: str, start: str, end: str, df: pd.DataFrame
    ):
    ...
    def _merge_layered_store(
        self, symbol: str, timeframe: str, new_out_with_dt: pd.DataFrame
    ):
        ...
        try:
            ...
            # Normalize columns
            out = new_out_with_dt.rename(
                columns={new_out_with_dt.columns[0]: "datetime"}
            )
                ...
                try:
                    ...
                    for c in ["datetime", "Date", "date", "timestamp", "time"]:
                        if c in existing.columns:
                            dt_col = c
                            break
                    existing[dt_col] = pd.to_datetime(
                        existing[dt_col], utc=True, errors="coerce"
                    )
                    existing = (
                        existing.dropna(subset=[dt_col]).set_index(dt_col).sort_index()
                    )
                    existing = existing[
                        [
                            c
        ...
        for prov in providers:
            ...
            if prov == "yfinance":
                ...
                df = self.fetch_dukascopy(
                    symbol, timeframe, start, end, allow_network=allow_network
                )
            elif prov == "alpha_vantage":
                df = self.fetch_alpha_vantage_fx(
                    symbol, timeframe, start, end, allow_network=allow_network
                )
            elif prov == "stooq":
