...
class DependencyInstaller:
    ...
    def print_summary(self, installed: int, skipped: int, failed: int):
        ...
        if success_rate >= 90:
            ...
            print(
                "❌ Poor. Many features may not work. Please resolve failed installations."
            )
    ...
    def run_full_installation(self, include_optional: bool = False) -> bool:
        ...
        return (
            verification_success and failed < 3
        )  # Allow up to 2 failed non-critical packages
...
def main():
    ...
    try:
        choice = (
            input(
                "Install optional packages (MetaTrader5, ML libraries, etc.)? (y/N): "
            )
            .strip()
            .lower()
        )
