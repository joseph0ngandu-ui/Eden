...
def liquidity_grab_flags(
    df: pd.DataFrame, prev_sh: pd.Series, prev_sl: pd.Series
) -> Tuple[pd.Series, pd.Series]:
...
def displacement_flags(
    df: pd.DataFrame, atr_series: pd.Series, k: float = 1.5
) -> Tuple[pd.Series, pd.Series]:
...
def confluence_counts(
    ...
    df_merge = df_merge.merge(bull_fvg_df, on="time", how="left").merge(
        bear_fvg_df, on="time", how="left"
    )
    bull_fvg_fill = (
        (df_merge["low"] <= df_merge["bull_fvg_upper"])
        & (df_merge["close"] > df_merge["bull_fvg_lower"])
    ).fillna(False)
    bear_fvg_fill = (
        (df_merge["high"] >= df_merge["bear_fvg_lower"])
        & (df_merge["close"] < df_merge["bear_fvg_upper"])
    ).fillna(False)
    ...
    tf1, tf2 = (
        params.basis_tfs if hasattr(params, "basis_tfs") else params.bias_timeframes
    )
    ...
    conf_buy = (
        ...
        + bias_buy_tf1.astype(int)
        + bias_buy_tf2.astype(int)
    )
    ...
    conf_sell = (
        ...
        + bias_sell_tf1.astype(int)
        + bias_sell_tf2.astype(int)
    )
...
def simulate_trades(
        ...
        if cb >= params.min_confluences:
            if params.require_sweep and not bool(feats["bull_grab"].iat[i]):
                ...
                if params.require_bias_align:
                    if not (
                        close_i > float(sma_h1.iat[i])
                        and close_i > float(sma_h4.iat[i])
                    ):
                        pass
                    else:
    ...
    def trade_outcome(i: int, side: str):
        ...
        atr_val = (
            float(feats["atr_e"].iat[i])
            if not np.isnan(feats["atr_e"].iat[i])
            else float((df["high"] - df["low"]).rolling(14).mean().iat[i])
        )
        if side == "BUY":
    ...
    for idx, side in entries:
        ...
        feat_row = {
            ...
            "rsi_e": (
                float(feats["rsi_e"].iat[idx])
                if not np.isnan(feats["rsi_e"].iat[idx])
                else 50.0
            ),
            "wr": float(feats["wr"].iat[idx]),
            "dist_to_sma": (
                float(df["close"].iat[idx] - feats["sma_e"].iat[idx])
                if not np.isnan(feats["sma_e"].iat[idx])
                else 0.0
            ),
            "rsi_M15": (
                float(feats.get("rsi_M15", pd.Series([np.nan])).iat[idx])
                if "rsi_M15" in feats
                else 50.0
            ),
            "rsi_M30": (
                float(feats.get("rsi_M30", pd.Series([np.nan])).iat[idx])
                if "rsi_M30" in feats
                else 50.0
            ),
            "rsi_H1": (
                float(feats.get("rsi_H1", pd.Series([np.nan])).iat[idx])
                if "rsi_H1" in feats
                else 50.0
            ),
            "rsi_H4": (
                float(feats.get("rsi_H4", pd.Series([np.nan])).iat[idx])
                if "rsi_H4" in feats
                else 50.0
            ),
    ...
    for outcome in trades_seq:
        ...
        if conf >= params.confidence_threshold:
            ...
            if float(outcome.get("pnl", 0.0)) > 0:
                ...
                if params.stop_after_losses > 0 and consec_losses >= int(
                    params.stop_after_losses
                ):
                    if params.pause_bars_after_streak > 0:
                        cooldown_until = max(
                            cooldown_until, idx + int(params.pause_bars_after_streak)
                        )
                    consec_losses = 0
    ...
    score = (
        total_points
        + 20.0 * (win_rate - 50.0)
        - 200.0 * max(0, max_consec_losses - 2)
        - 0.5 * max(0, 10 - trades_ct)
    )
    stats = {
        "trades": trades_ct,
        "wins": wins,
...
def build_exec_frame(
    symbol: str, exec_tf: str, ctx_tfs: List[str], start: datetime, end: datetime
    ...
    for tf in ctx_tfs:
        ...
        feats = merge_asof_features(
            base[["time", "open", "high", "low", "close"]], cdf, suffix=tf
        )
        # Attach only derived columns
        derived_cols = [
            c for c in feats.columns if c.startswith("rsi_") or c.startswith("sma_")
        ]
...
def optimize_params(
    exec_df: pd.DataFrame, params: Params, point: float, n_trials: int = 20
) -> Params:
    if optuna is None:
    ...
    def objective(trial: optuna.trial.Trial) -> float:
        p = Params(
            ...
            require_bias_align=trial.suggest_categorical(
                "require_bias_align", [False, True]
            ),
        )
        conf_buy, conf_sell, feats = confluence_counts(exec_df, p)
        res, _ = simulate_trades(
            exec_df, conf_buy, conf_sell, feats, p, point, use_ml=True, force_ml=True
        )
        stats = res["stats"]
    ...
    best = Params(
        ...
        bias_timeframes=params.bias_timeframes,
        cooldown_bars_after_loss=bp.get(
            "cooldown_loss_bars", getattr(params, "cooldown_bars_after_loss", 0)
        ),
        stop_after_losses=bp.get(
            "stop_after_losses", getattr(params, "stop_after_losses", 0)
        ),
        pause_bars_after_streak=bp.get(
            "pause_bars_after_streak", getattr(params, "pause_bars_after_streak", 0)
        ),
        require_sweep=bp.get("require_sweep", getattr(params, "require_sweep", False)),
        require_bias_align=bp.get(
            "require_bias_align", getattr(params, "require_bias_align", False)
        ),
    )
    return best


def save_best_to_config(
    config_path: Path, params: Params, exec_tf: str, ctx_tfs: List[str]
):
    if yaml is None:
...
def main():
    ...
    parser.add_argument(
        "--continuous",
        action="store_true",
        help="Run repeated optimization rounds until convergence",
    )
    ...
    try:
            ...
            if optuna is None:
                ...
                if args.continuous:
                    ...
                    for r in range(args.rounds):
                        ...
                        conf_buy_tmp, conf_sell_tmp, feats_tmp = confluence_counts(
                            exec_df, cand
                        )
                        res_tmp, _ = simulate_trades(
                            exec_df,
                            conf_buy_tmp,
                            conf_sell_tmp,
                            feats_tmp,
                        ...
                        score_tmp = float(
                            res_tmp["stats"].get(
                                "score", res_tmp["stats"].get("total_pnl_points", 0.0)
                            )
                        )
        ...
        print("=========================")
        print(
            f"Symbol: {symbol}  Exec TF: {exec_tf}  Window: {args.days}d  Bars: {len(exec_df)}"
        )
        print(
            f"Trades: {stats['trades']}  Wins: {stats['wins']}  Losses: {stats['losses']}  Win%: {stats['win_rate']:.2f}%  Max L-streak: {stats.get('max_consecutive_losses', 0)}"
        )
        print(
            f"Total PnL: {stats['total_pnl_points']:.2f} points (point={stats['point']})  Score: {stats.get('score', 0.0):.2f}"
        )
        print(
            f"Params: min_confs={best_params.min_confluences} rr={best_params.rr} conf>={best_params.confidence_threshold}"
        )
        ...
        if args.save_best_config:
            save_best_to_config(
                Path(args.save_best_config), best_params, exec_tf, ctx_tfs
            )
    finally:
        mt5.shutdown()
